...
static int lua_normalize(lua_State* L)
{
	...
	else if (isVec4(L, 1), isVec4(L, 2))
	{
		glm::vec4 a = toVec4(L, 1);

		pushVec4(L, glm::normalize(a));
	}
	else WC_ERROR("Unknown argument type for normalize");
...
static int lua_min(lua_State* L)
{
	...
	else WC_ERROR("Unknown argument type for min");
...
static int lua_max(lua_State* L)
{
	...
	else WC_ERROR("Unknown argument type for max");
...
static int lua_length(lua_State* L)
{
	...
	else WC_ERROR("Unknown argument type for length");
...
static int lua_distance(lua_State* L)
{
	...
	else WC_ERROR("Unknown argument type for distance");
...
{
	if (!isVec3(L, 1) || !isVec3(L, 2))
	{
		WC_ERROR("cross is only for vec3 types")
			return 0;
	}
...
static int lua_dot(lua_State* L)
{
	...
	else WC_ERROR("Unknown argument type for dot");
...
int Script::Load(const ScriptBinary& script)
{
