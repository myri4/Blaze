...
void EditorInstance::RenderEntity(flecs::entity entt, glm::mat4& transform)
{
...
void EditorInstance::Update() { m_Scene.Update(); }

void EditorInstance::UI_Editor()
{
...
void EditorInstance::UI_SceneProperties()
{
...
void EditorInstance::EntityRightClickMenu(const flecs::entity& entity)
{
...
void EditorInstance::EntityReorderSeparator(const flecs::entity& entity)
{
...
void EditorInstance::DisplayEntity(const flecs::entity& entity)
{
...
void EditorInstance::EditComponent(const std::string& name, UIFunc uiFunc)
{
...
void EditorInstance::UI_Properties()
{
...
void EditorInstance::UI_Console()
{
	gui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0.f, 0.f));
...
void EditorInstance::UI_DebugStats()
{
...
void EditorInstance::UI_StyleEditor(ImGuiStyle* ref)
{
...
void EditorInstance::WindowButtons()
{
	// Buttons
...
void EditorInstance::AddSceneToList(const std::string& path)
{
	if (std::find(savedProjectScenes.begin(), savedProjectScenes.end(), path) == savedProjectScenes.end())
		savedProjectScenes.push_back(path);
}

void EditorInstance::RemoveSceneFromList(const std::string& filepath) { std::erase(savedProjectScenes, filepath); }

bool EditorInstance::SceneExistInList(const std::string& path)
{
...
void EditorInstance::AddProjectToList(const std::string& path)
{
	if (std::find(savedProjectPaths.begin(), savedProjectPaths.end(), path) == savedProjectPaths.end())
		savedProjectPaths.push_back(path);
}

void EditorInstance::RemoveProjectFromList(const std::string& filepath) { std::erase(savedProjectPaths, filepath); }

bool EditorInstance::ProjectExistInList(const std::string& pName)
{
...
void EditorInstance::ResetProject()
{
	ProjectName = "";
	ProjectRootPath = "";
	ProjectFirstScene = "";
...
bool EditorInstance::IsProject(const std::string& filepath) { return std::filesystem::exists(filepath + "/settings" + PROJECT_SETTINGS_EXTENSION); }
std::string EditorInstance::GetProjectSettingsPath() { return ProjectRootPath + "/settings" + PROJECT_SETTINGS_EXTENSION; }
std::string EditorInstance::GetProjectUserSettingsPath() { return ProjectRootPath + "/settings" + PROJECT_USER_SETTINGS_EXTENSION; }

void EditorInstance::SaveProjectData()
{
...
void EditorInstance::SaveProject()
{
	SaveProjectData();
	m_Scene.Save(); // @TODO: This should save all opened scenes
}

bool EditorInstance::LoadProject(const std::string& filepath)
{
	try
	{
...
void EditorInstance::CreateProject(const std::string& filepath, const std::string& pName)
{
...
void EditorInstance::DeleteProject(const std::string& filepath) // @TODO: This function should accept project index from savedProjectPaths
{
	if (std::filesystem::exists(filepath))
	{
		if (IsProject(filepath))
...
void EditorInstance::RenameProject(const std::string& newName)
{
	if (newName.empty())
	{
...
bool EditorInstance::ProjectExists() { return !ProjectName.empty() && !ProjectRootPath.empty(); }

// [SETTINGS MANAGING]

void EditorInstance::SaveSettings()
{
	YAML::Node data;
...
void EditorInstance::LoadSettings()
{
	try
	{
