...
namespace wc
{
	...
	struct ShaderSpecification
	{
		...
		//bool blending = true; //@TODO: this should be handled by the renderpass creation
	...
	public:
		...
		Shader CreateShader(ShaderSpecification spec, const GraphicsPass& rp)
		{
			auto& shader = Shaders.emplace_back();

			ShaderCreateInfo createInfo;
			createInfo.binaries[0] = std::move(spec.binaries[0]);
			createInfo.binaries[1] = std::move(spec.binaries[1]);
			...
			createInfo.dynamicDescriptorCount = spec.dynamicDescriptorCount;
				...
				else if (rawPass->type == PassType::Compute)
				{
					...
					for (auto& c : encoder.GetEncodeBuffer())
					{
						...
						case CommandType::BindDescriptorSet:
						{
							...
							vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pass->shader.GetPipelineLayout(), pCmd->setNumber, 1, (VkDescriptorSet*)&pCmd->descriptorSet, 0, nullptr);
						}
						break;
						...
						{
							auto* pCmd = static_cast<CMD_PushConstants*>(command);
							vkCmdPushConstants(cmd, pass->shader.GetPipelineLayout(), VK_SHADER_STAGE_COMPUTE_BIT, pCmd->offset, pCmd->size, pCmd->data);
							free(pCmd->data);
						}
						break;
						}
